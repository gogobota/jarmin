...
def generate_contours(work_dir, output_pbf, bbox=None, polygon_file=None):
    pyhgtmap_bin = BASE_DIR / ".venv" / "bin" / "pyhgtmap"
    if not pyhgtmap_bin.exists():
        raise RuntimeError(f"pyhgtmap not found at {pyhgtmap_bin}. Did you run download.sh to create the venv?")

    hgt_cache = work_dir / "hgt"
    hgt_cache.mkdir(parents=True, exist_ok=True)
    
    # We create a temporary output prefix so pyhgtmap doesn't guess the filename
    ...
    generated_files = list(work_dir.glob("generated_contours*.osm.pbf"))
    if not generated_files:
        raise RuntimeError("pyhgtmap failed to generate any contour PBF files.")
    
    # If it generated multiple files, we'd need to merge them. For simplicity, we assume one combined file or we merge them.
    if len(generated_files) > 1:
        print("  Merging multiple contour files generated by pyhgtmap...")
        cmd_merge = [str(OSMCONVERT_BIN)] + [str(f) for f in generated_files] + [f"-o={output_pbf}"]
        run_cmd(cmd_merge)
...
def main():
    ...
    parser.add_argument("--generate-elevation", action="store_true", help="Automatically download SRTM data and generate contour lines for the region")
    ...
    run_pipeline(
        ...
        generate_elevation=args.generate_elevation,
        dem_dir=args.dem
    )
